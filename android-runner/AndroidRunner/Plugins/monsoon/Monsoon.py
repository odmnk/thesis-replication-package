...
class Monsoon(Profiler):
    def __init__(self, config, paths):
        ...
        self.paths = paths
        self.profile = False
        self.data_points = ["run", "energy_joules", "duration_ms", "error_flag"]
...
#        time.sleep(5)
    ...
    def collect_results(self, device, run):
        ...
        with open(op.join(self.output_dir, filename), 'w+') as f:
            ...
            writer.writerow([run, self.results[0], round(self.results[1]*1000), self.results[2]])
    ...
    def add_rows(self, row, temp_row, rows, dir):
        ...
        for i in range(repetition_count):
            temp_row.update({self.data_points[0]: row[self.data_points[0]][i], self.data_points[1]: row[self.data_points[1]][i], self.data_points[2]: row[self.data_points[2]][i], self.data_points[3]: row[self.data_points[3]][i]})
