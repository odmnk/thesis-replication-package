...
class Experiment(object):
    ...
    def finish_run(self, current_run):
        if not self.run_error:
            ...
            print("We skip the previous run")
    ...
    def after_launch(self, device, path, run, *args, **kwargs):
        ...
        time.sleep(5)
        current_activity = device.shell("dumpsys window windows | grep -E 'mCurrentFocus|mFocusedApp'")
        print("Current activity: ", current_activity)
        didnt_load = "FirstRunActivity" in current_activity
        print("Chrome didn't load: ", didnt_load)

        # if "FirstRunActivity" in current_activity:
        print("error_run" , self.run_error)
        if didnt_load:
            self.run_error = True
            print("Error in after launch: First Run Acitivity spotted, we skip this run")
    ...
    def stop_profiling(self, device, path, run, *args, **kwargs):
        try:
            ...
            print(str(error))
            self.run_error = True
            print("Stop Profiling: We drop this run and continue")
    ...
    def after_run(self, device, path, run, *args, **kwargs):
        ...
        if not self.run_error:
            self.profilers.collect_results(device, run)
        else:
            print("After run: We skip this run as it returned an error")
